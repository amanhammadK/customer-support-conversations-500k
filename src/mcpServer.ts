...
export const server = new Server(
  { name: "customer-support-conversations", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
...
const TOOLS = [
  {
    name: "query_dataset",
    description:
      "Filter the conversation dataset. Pass either a flat object of exact-match filters (values starting and ending with % do substring match, arrays match any-of) or an array of {field, op, value} with ops =, !=, >, >=, <, <=, contains, starts_with, ends_with, in, between, is_null, is_not_null. Supports sortBy/sortDir and pagination via limit/offset.",
    inputSchema: {
      type: "object",
      properties: {
